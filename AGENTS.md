...
- If `plan next` recommends `Set roadmap focus for this worktree`, run `zenith focus show --json` to see candidates, then `zenith focus set <roadmap-id> --json` to bind the current worktree to a roadmap before continuing.
